...
public class MyAlbumActivity extends BaseActivity implements OnClickListener, RecyclerListener, OnChangeStateListener{
	...
	private Handler mHandler = new Handler(){
		...
		public void handleMessage(Message msg) {
			...
			case GlobalParam.MSG_SHOW_HEADER_IMG://显示背景图
				if(mHeaderIcon!=null){
					...
					if (mHeadUrl!=null && !mHeadUrl.equals("")) {
						...
						mImageLoader.getBitmap(mContext, mHeaderIcon, null,mHeadUrl, 0,true,false,false);
					}else{
						mHeaderIcon.setImageResource(R.drawable.contact_default_header);
					}
				}
			...
			case MSG_SHOW_PROFILE_HEADER:
				if(mLogin!=null ){
					mImageLoader.getBitmap(mContext, mHeaderBg, null,mLogin.cover, 0,true,false,false);
				}
				break;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if(mToUserID == null || mToUserID.equals("")){
			mToUserID = WeiYuanCommon.getUserId(mContext);
		}
	...
	private void initCompent(){
		if(mToUserID.equals(WeiYuanCommon.getUserId(mContext))){
		...
		if (mListView.getHeaderViewsCount()==0) {
			...
			if(mToUserID!=null && !mToUserID.equals("") && mToUserID.equals(WeiYuanCommon.getUserId(mContext))){
			...
			public void run() {
				try {
					if (loadType == GlobalParam.LIST_LOAD_FIRST) {
						...
						LoginResult loginResult = WeiYuanCommon.getWeiYuanInfo().getUserInfo(mToUserID);
					...
					if (isExitsData) {
						...
						mMyAlbum = WeiYuanCommon.getWeiYuanInfo().myHomeList(page,mToUserID);
