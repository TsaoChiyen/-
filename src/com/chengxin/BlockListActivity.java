...
public class BlockListActivity extends BaseActivity implements OnItemClickListener, OnClickListener{
	...
	public static final String CANCEL_ACTION = "weiyuan_cancel_action";
	public static final String CANCEL_FOLLOW_ACTION = "weiyuan_cancel_follow_action";
						...
						public void run() {
							try {
								if(type == GlobleType.BLOCKLISTACTIVITY_BLOCK_TYPE){
									mUser = WeiYuanCommon.getWeiYuanInfo().getBlockList(/*currentPage*/);
								}
			...
			new Thread(){
				...
				public void run(){
					try {
						...
						if(mType == 1){
							state = WeiYuanCommon.getWeiYuanInfo().addfocus(fuid/*,isFollow*/);
						}else{
							state=WeiYuanCommon.getWeiYuanInfo().cancelBlock(fuid);
						}
