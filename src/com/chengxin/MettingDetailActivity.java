...
public class MettingDetailActivity extends BaseActivity {
	 ...
	 * 定义全局变量
	 */
	public final static String ACTION_SHOW_NEW_MEETING_TIP ="weiyuan_action_show_new_meeting_tip";
	public final static String ACTION_HIDE_NEW_MEETING_TIP ="weiyuan_action_hide_new_meeting_tip";
		...
		new Thread(){
			public void run() {
				try {
					...
					MeetingItem item = WeiYuanCommon.getWeiYuanInfo().mettingDetail(mMetId);
					mBaseHandler.sendEmptyMessage(BASE_HIDE_PROGRESS_DIALOG);
					WeiYuanCommon.sendMsg(mHandler, GlobalParam.MSG_SHOW_LOAD_DATA,item);
	...
	private void applyMeeting(final String reseon){
		if(WeiYuanCommon.verifyNetwork(mContext)){
			new Thread(){
				...
				public void run(){
					try {
						WeiYuanState state = WeiYuanCommon.getWeiYuanInfo().applyMeeting(mMetId,reseon);
						mBaseHandler.sendEmptyMessage(BASE_HIDE_PROGRESS_DIALOG);
						WeiYuanCommon.sendMsg(mHandler,GlobalParam.MSG_CHECK_APPLY_METTING,state);
					} catch (WeiYuanException e) {
						e.printStackTrace();
