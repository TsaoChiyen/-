	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		if(userList != null && userList.size() != 0){
			...
			if(count == 1){
				...
				mImageLoader.getBitmap(mContext, holder.mHeaderView, null, userList.get(0).headsmall,
						0, false, true,false);
			}else {
				holder.mGroupHeaderLayout.setVisibility(View.VISIBLE);
				holder.mHeaderView.setVisibility(View.GONE);
				...
				for (int i = 0; i < row; i++) {
					...
					if(single && i == 0){
						...
						mImageLoader.getBitmap(mContext, imageView, null, userList.get(0).headsmall,
								0, false, true,false);
						...
						for (int j = 0; j < 2; j++) {
							...
							if(single){
								mImageLoader.getBitmap(mContext, imageView, null,
										userList.get(2 * i + j - 1).headsmall, 0, false, true,false);
							}else {
								mImageLoader.getBitmap(mContext, imageView, null, 
										userList.get(2 * i + j).headsmall, 0, false, true,false);
							}
							imageView.setLayoutParams(new LinearLayout.LayoutParams(LayoutParams.FILL_PARENT, LayoutParams.FILL_PARENT));
							layout.addView(imageView);
							outLayout.addView(layout);
