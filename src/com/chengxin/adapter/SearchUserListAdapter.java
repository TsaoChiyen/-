	...
	public View getView(int position, View convertView, ViewGroup arg2) {
		...
		if(user.mIsRoom == 100){//不是房间消息
			...
			if(user.headsmall != null && !user.headsmall.equals("")){
				mImageLoader.getBitmap(mContext, holder.mHeaderView, null, user.headsmall, 0, false, true,false);
			}
		}else{
							...
							for (int j = 0; j < 2; j++) {
								...
								if(single){
									mImageLoader.getBitmap(mContext, imageView, null,
											headUrlArray[(2 * i + j - 1)], 0, false, true,false);
								}else {
									mImageLoader.getBitmap(mContext, imageView, null, 
											headUrlArray[(2 * i + j)], 0, false, true,false);
								}
								imageView.setLayoutParams(new LinearLayout.LayoutParams(LayoutParams.FILL_PARENT, LayoutParams.FILL_PARENT));
								layout.addView(imageView);
								outLayout.addView(layout);
