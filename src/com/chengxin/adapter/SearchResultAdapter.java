	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		try {
			if (convertView == null || ((ViewHolder) convertView.getTag()).mTag != position) {
				...
				holder.mDistanceTextView = (TextView)convertView.findViewById(R.id.distance);
				if(mIsShowSign){
			...
			if(isShowValidBtn){
				...
				if(item.headsmall!=null && !item.headsmall.equals("")){
					mImageLoader.getBitmap(ctx, holder.mHeadrIcon,null,item.headsmall,0,false,true,false);
				}
			}
			holder.mDistanceTextView.setText(FeatureFunction.getDistance(item.distance));
