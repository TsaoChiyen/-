...
public class FriendsLoopDetailActivity extends BaseActivity implements OnTouchListener, OnPageChangeListener{
	...
	private void initCompent(){
		...
		mContentTextView.setOnLongClickListener(new OnLongClickListener() {
			...
			public boolean onLongClick(View v) {
				WeiYuanCommon.sendMsg(mHandler, GlobalParam.MSG_SHOW_FRIENDS_FAVORITE_DIALOG,1,0 );
				return true;
			}
		});
	...
	private void setContent(){
		...
		if(item.uid.equals(WeiYuanCommon.getUserId(mContext))){
			mDelBtn.setVisibility(View.VISIBLE);
		}
		if(item.headsmall!=null && !item.headsmall.equals("")){
			mImageLoader.getBitmap(mContext, mHeaderIcon, null,item.headsmall,0, false,true,false);
		}
				...
				for (int j = 0; j < 3; j++) {
					...
					if(pos < item.listpic.size()){
						...
						view.setOnLongClickListener(new OnLongClickListener() {
							...
							@Override 
							public boolean onLongClick(View v) {
								WeiYuanCommon.sendMsg(mHandler, GlobalParam.MSG_SHOW_FRIENDS_FAVORITE_DIALOG,2,pos );
								return true;
							}
						});
						...
						if(!TextUtils.isEmpty(item.listpic.get(pos).smallUrl)){
							...
							mImageLoader.getBitmap(mContext, imageView, null, item.listpic.get(pos).smallUrl, 0, false, false,false);
						}else {
							imageView.setImageResource(R.drawable.normal);
						}
					...
					for (int j = 0; j <6; j++) {
						...
						if(pos < item.praiselist.size()){
							...
							if(zanList.get(pos).headsmall!=null && !zanList.get(pos).headsmall.equals("")){
								mImageLoader.getBitmap(mContext, tv, null, zanList.get(pos).headsmall, 0, false, true,false);
							}
			...
			if (commentList!=null && commentList.size()>0) {
				...
				for (int i = 0; i < commentList.size(); i++) {
					...
					if(commentList.get(i).headsmall!=null && !commentList.get(i).headsmall.equals("")){
						mImageLoader.getBitmap(mContext, userIcon, null, commentList.get(i).headsmall, 0, false, true,false);
					}
		...
		new Thread(){
			public void run() {
				try {
					...
					if(shareDetail !=null){
						WeiYuanCommon.sendMsg(mHandler, GlobalParam.MSG_SHOW_LOAD_DATA,shareDetail);
					}
		...
		new Thread(){
			public void run() {
				try {
					...
					WeiYuanState status = WeiYuanCommon.getWeiYuanInfo().shareReply(item.id,item.uid,content);
		...
		new Thread(){
			public void run() {
				try {
					WeiYuanCommon.sendMsg(mBaseHandler, BASE_SHOW_PROGRESS_DIALOG,
							mContext.getResources().getString(R.string.send_request));
					WeiYuanState status = WeiYuanCommon.getWeiYuanInfo().sharePraise(item.id);
					if(status!=null && status.code == 0){
						getShareDetail();
					}else{
		...
		new Thread(){
			public void run() {
				try {
					WeiYuanCommon.sendMsg(mBaseHandler, BASE_SHOW_PROGRESS_DIALOG, 
							mContext.getResources().getString(R.string.send_request));
					WeiYuanState status = WeiYuanCommon.getWeiYuanInfo().deleteShare(item.id);
