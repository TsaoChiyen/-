...
public abstract class HttpHeaderFactory {
    ...
    public String getWeiboAuthHeader(String method, String url, WeiYuanParameters params){
    	try {
    		final long timestamp = System.currentTimeMillis() / 1000;
            final long nonce = timestamp + (new Random()).nextInt();
            // step 2: authParams�������ô���1.���ܴ�һ���� 2.������Authorizationͷ��
            WeiYuanParameters authParams = this.generateAuthParameters(nonce, timestamp);
            // ������ڼ���signature�ģ�����
            WeiYuanParameters signatureParams = this.generateSignatureParameters(authParams, params, url);
    ...
    private String generateAuthSignature(final String method, WeiYuanParameters signatureParams,
            final String url) {
    ...
    private WeiYuanParameters generateSignatureParameters(WeiYuanParameters authParams,
    		WeiYuanParameters params, String url){
    	WeiYuanParameters signatureParams = new WeiYuanParameters();
        signatureParams.addAll(authParams);
        signatureParams.addAll(params);
        this.parseUrlParameters(url, signatureParams);
        WeiYuanParameters lsp = generateSignatureList(signatureParams);
        return lsp;
    }

    private WeiYuanParameters generateAuthParameters(long nonce, long timestamp){
    	WeiYuanParameters authParams = new WeiYuanParameters();
    ...
    public abstract WeiYuanParameters generateSignatureList(WeiYuanParameters bundle);

    // add additional parameters to des key-value pairs,support to expanding
    // params
    public abstract void addAdditionalParams(WeiYuanParameters des, WeiYuanParameters src);

    // ����url�в����,�洢��signatureBaseParams
    public void parseUrlParameters(String url, WeiYuanParameters signatureBaseParams){
    ...
    public static String encodeParameters(WeiYuanParameters postParams, String splitter, boolean quot) {
