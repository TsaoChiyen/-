...
public class UserInfoActivity extends BaseActivity  implements OnClickListener{
	...
	private void initComponent(){
		if(mLogin!=null && mLogin.uid.equals(WeiYuanCommon.getUserId(mContext))){
		...
		if(mType != 2){
			...
			WeiYuanCommon.sendMsg(mBaseHandler, BASE_SHOW_PROGRESS_DIALOG,mContext.getResources().getString( R.string.add_more_loading));
	...
	private void showProfile(){
		...
		if(mLogin.uid.equals(WeiYuanCommon.getUserId(mContext))){
			mRightBtn.setVisibility(View.GONE);
		...
		if(mLogin != null){
			if(mLogin.headsmall != null && !mLogin.headsmall.equals("")){
				mImageLoader.getBitmap(mContext, mHeaderView, null, mLogin.headsmall, 0, false, true,false);
			}
			//showAddr();
	...
	public void onClick(View v) {
		...
		case R.id.add_friends://添加好友
			if(mLogin!=null){
				if(mLogin.isfriend == 0){
					WeiYuanCommon.sendMsg(mBaseHandler, BASE_SHOW_PROGRESS_DIALOG,R.string.add_more_loading);
					createDialog(mContext,
							mContext.getResources().getString(R.string.request_doing)
							,2,mContext.getResources().getString(R.string.send));
				}
	...
	private void addBlock(){
		if(WeiYuanCommon.verifyNetwork(mContext)){
			new Thread(){
				...
				public void run(){
					try {
						WeiYuanState state = WeiYuanCommon.getWeiYuanInfo().addBlock(mLogin.uid);
	...
	private void deleteFriend(){
		if(WeiYuanCommon.verifyNetwork(mContext)){
			new Thread(){
				...
				public void run(){
					try {
						WeiYuanState state = WeiYuanCommon.getWeiYuanInfo().cancleFriends(mLogin.uid);
						if(state != null && state.code == 0){
							...
							List<NewFriendItem> list = WeiYuanCommon.getNewFriendItemResult(mContext);
							if(list!=null && list.size()>0){
								for (int i = 0; i < list.size(); i++) {
									if(list.get(i).uid.equals(mLogin.uid)){
										list.get(i).type = 0;
										WeiYuanCommon.saveNewFriendsResult(mContext, list,0);
										break;
									}
								}
							}
	...
	private void addFriend(final String reseon){
		if(WeiYuanCommon.verifyNetwork(mContext)){
			new Thread(){
				...
				public void run(){
					try {
						WeiYuanState state = WeiYuanCommon.getWeiYuanInfo().applyFriends(WeiYuanCommon.getUserId(mContext), mLogin.uid,reseon);
						if(state != null && state.code == 0){
							String hintMsg = "";
							int checkStatus = 4;
							...
							WeiYuanCommon.sendMsg(mHandler,GlobalParam.HIDE_PROGRESS_DIALOG,hintMsg,4);
			...
			new Thread(){
				...
				public void run(){
					try {
						...
						if(searchType == 1){
							loginResult = WeiYuanCommon.getWeiYuanInfo().getUserInfo(uid);
						}else if(searchType == 2){
							loginResult = WeiYuanCommon.getWeiYuanInfo().getUserByName(mSearchName);
						}
	...
	private void cancelAddFocus(){
		...
		new Thread(){
			public void run() {
				try {
					WeiYuanCommon.sendMsg(mBaseHandler, BASE_SHOW_PROGRESS_DIALOG, R.string.commit_dataing);
					WeiYuanState state = WeiYuanCommon.getWeiYuanInfo().addfocus(mLogin.uid);
		...
		new Thread(){
			public void run() {
				try {
					...
					LoginResult state = WeiYuanCommon.getWeiYuanInfo().setStar(mLogin.uid);
					WeiYuanCommon.sendMsg(mHandler, GlobalParam.MSG_CHECK_STAR,state);
					mBaseHandler.sendEmptyMessage(BASE_HIDE_PROGRESS_DIALOG);
				} catch (WeiYuanException e) {
					e.printStackTrace();
					WeiYuanCommon.sendMsg(mBaseHandler, BASE_MSG_TIMEOUT_ERROR, 
	...
	private void checkFriendsNotify(Login login,int type){
		...
		mUserList.add(new NewFriendItem(login.phone,login.uid,login.nickname,login.headsmall,
				type,"",WeiYuanCommon.getUserId(mContext),1));

		//获取系统中保存的新的朋友
		List<NewFriendItem> lastNewFriendsList = WeiYuanCommon.getNewFriendItemResult(mContext);
		...
		WeiYuanCommon.saveNewFriendsResult(mContext, mUserList,mUserList.size());
	}
