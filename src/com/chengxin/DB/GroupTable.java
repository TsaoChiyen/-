...
public class GroupTable {
	...
	public boolean update(Group group) {
		...
		try {
			mDBStore.update(TABLE_NAME, allPromotionInfoValues, COLUMN_GROUP_ID + " = '" + group.id + "' AND " + COLUMN_LOGIN_ID + "='" + WeiYuanCommon.getUserId(BMapApiApp.getInstance()) + "'", null);
			return true;
		} catch (SQLiteConstraintException e) {
			e.printStackTrace();
		}			
	...
	public boolean delete(int groupId) {
		try {
			mDBStore.delete(TABLE_NAME, COLUMN_GROUP_ID + " = " + groupId + " AND " + COLUMN_LOGIN_ID + "='" + WeiYuanCommon.getUserId(BMapApiApp.getInstance()) + "'", null);
			return true;
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public Group query(int groupId){
		...
		try {
			cursor = mDBStore.rawQuery("SELECT * FROM " + TABLE_NAME 
					...
					+ COLUMN_LOGIN_ID + "='" + WeiYuanCommon.getUserId(BMapApiApp.getInstance()) + "'", null);
			if (cursor != null) {
	...
	public List<Group> queryGroup(int initId) {
		...
		Cursor cursor = null;
		try {
			String sql = "SELECT * FROM " + TABLE_NAME + " WHERE " + COLUMN_LOGIN_ID + "='" + WeiYuanCommon.getUserId(BMapApiApp.getInstance()) + "' AND " + COLUMN_GROUP_ID + ">=" + initId ;
			Log.e("query", sql);
			cursor = mDBStore.rawQuery(sql, null);
			if (cursor != null) {
