...
public class MessageTable {
	...
	/**
	 * content, fromId, toId, sessionId, pullTime, sendTime, voiceTime, type, readState, sendState, sendType, currentUser, primary key(fromId, toId, sendTime, currentUser)
	 */
	...
	public static String getCreateTableSQLString() {
		if (null == mSQLCreateWeiboInfoTable) {
			...
			columnNameAndType.put(COLUMN_VIDEO_URL, COLUMN_TEXT_TYPE);
			columnNameAndType.put(COLUMN_VIDEO_TIME, COLUMN_TEXT_TYPE);
	...
	public void insert(List<MessageInfo> messages) {
		...
		for (MessageInfo message : messageList) {
			...
			allPromotionInfoValues.put(COLUMN_LOGIN_ID, WeiYuanCommon.getUserId(BMapApiApp.getInstance()));
			...
			allPromotionInfoValues.put(COLUMN_VIDEO_URL, message.videoUrl);
			allPromotionInfoValues.put(COLUMN_VIDEO_TIME, message.videotime);
	...
	public void delete(MessageInfo message) {
		mDBStore.delete(TABLE_NAME, COLUMN_TAG + "='" + message.tag + "' AND " + COLUMN_LOGIN_ID + "='" + WeiYuanCommon.getUserId(BMapApiApp.getInstance()) + "'", null);
	}

	public boolean delete(String toId, int type) {
		try {
			if (type == 300) {
				mDBStore.delete(TABLE_NAME, COLUMN_TO_ID + "='" + toId + "'" + " AND " + COLUMN_TYPE + "=300" + " AND " + COLUMN_LOGIN_ID + "='" + WeiYuanCommon.getUserId(BMapApiApp.getInstance())
						+ "'", null);
			} else if (type == 500) {
				mDBStore.delete(TABLE_NAME, COLUMN_TO_ID + "='" + toId + "'" + " AND " + COLUMN_TYPE + "=500" + " AND " + COLUMN_LOGIN_ID + "='" + WeiYuanCommon.getUserId(BMapApiApp.getInstance())
						+ "'", null);
			} else {
				mDBStore.delete(TABLE_NAME, (COLUMN_FROM_UID + "='" + toId + "' or " + COLUMN_TO_ID + "='" + toId + "'") + " AND " + COLUMN_TYPE + "=100" + " AND " + COLUMN_LOGIN_ID + "='"
						+ WeiYuanCommon.getUserId(BMapApiApp.getInstance()) + "'", null);
			}
			return true;
		} catch (Exception e) {
			e.printStackTrace();
	...
	public boolean updateMessage(MessageInfo message) {
		...
		try {
			mDBStore.update(TABLE_NAME, allPromotionInfoValues, COLUMN_TAG + "='" + message.tag + "' AND " + COLUMN_LOGIN_ID + "='" + WeiYuanCommon.getUserId(BMapApiApp.getInstance()) + "'", null);
			return true;
		} catch (SQLiteConstraintException e) {
			e.printStackTrace();
		}
	...
	public boolean updateVoiceContent(String tag, String content) {
		...
		try {
			String sql = "UPDATE " + TABLE_NAME + " SET " + COLUMN_CONTENT + "='" + content + "' WHERE " + COLUMN_TAG + "='" + tag + "' AND " + COLUMN_TYPE + "=100 AND " + COLUMN_LOGIN_ID + "='"
					+ WeiYuanCommon.getUserId(BMapApiApp.getInstance()) + "'";
	...
	public boolean updateReadState(String id, int type) {
		...
		try {
			if (type == 0)
				type = 600;
			String sql = "";
			if (type == MessageType.SINGLE_CHAT || type == MessageType.SERVICE_CHAT || type == MessageType.SUB_CHAT) {
				sql = "UPDATE " + TABLE_NAME + " SET " + COLUMN_READ_STATE + "=1 WHERE " + (COLUMN_FROM_UID + "='" + id + "' OR " + COLUMN_TO_ID + "='" + id) + "' AND " + COLUMN_TYPE + "=" + type
						+ " AND " + COLUMN_LOGIN_ID + "='" + ResearchCommon.getUserId(BMapApiApp.getInstance()) + "'";
			} else if (type == MessageType.GROUP_CHAT) {
				sql = "UPDATE " + TABLE_NAME + " SET " + COLUMN_READ_STATE + "=1 WHERE " + COLUMN_TO_ID + "='" + id + "' AND " + COLUMN_TYPE + "=" + type + " AND " + COLUMN_LOGIN_ID + "='"
						+ ResearchCommon.getUserId(BMapApiApp.getInstance()) + "'";
			} else if (type == MessageType.MEETING_CHAT) {
				sql = "UPDATE " + TABLE_NAME + " SET " + COLUMN_READ_STATE + "=1 WHERE " + COLUMN_TO_ID + "='" + id + "' AND " + COLUMN_TYPE + "=" + type + " AND " + COLUMN_LOGIN_ID + "='"
						+ ResearchCommon.getUserId(BMapApiApp.getInstance()) + "'";
			}
			mDBStore.execSQL(sql);
	...
	public boolean updatePrivateReadState(String id) {
		...
		try {
			String sql = "UPDATE " + TABLE_NAME + " SET " + COLUMN_READ_STATE + "=1 WHERE " + (COLUMN_FROM_UID + "='" + id + "' OR " + COLUMN_TO_ID + "='" + id) + "' AND " + COLUMN_TYPE + "=100 AND "
					+ COLUMN_LOGIN_ID + "='" + WeiYuanCommon.getUserId(BMapApiApp.getInstance()) + "'";
			mDBStore.execSQL(sql);
			return true;
		} catch (SQLiteConstraintException e) {
			e.printStackTrace();
	...
	public MessageInfo query(String tag) {
		...
		try {
			...
			if (cursor != null) {
				...
				int indexVideoUrl = cursor.getColumnIndex(COLUMN_VIDEO_URL);
				int indexVideoTime = cursor.getColumnIndex(COLUMN_VIDEO_TIME);
				...
				message.videoUrl = cursor.getString(indexVideoUrl);
				message.videotime = cursor.getInt(indexVideoTime);
	...
	public List<MainSearchEntity> queryByContent(String searchContent, String toId, int type) {
		...
		try {
			...
			if (type == 0) {// 查询所有的数据（包括单聊和群聊）
				querySql = "SELECT * FROM " + TABLE_NAME + " WHERE " + COLUMN_LOGIN_ID + "='" + WeiYuanCommon.getUserId(BMapApiApp.getInstance()) + "'" + " AND " + COLUMN_CONTENT + " like '%"
						+ searchContent + "%' " + " AND " + COLUMN_MESSAGE_TYPE + " =" + MessageType.TEXT;
			} else {
				if (type == 100 && (toId != null && !toId.equals(""))) {
					querySql = "SELECT * FROM " + TABLE_NAME + " WHERE (" + COLUMN_FROM_UID + "='" + toId + "' or " + COLUMN_TO_ID + "='" + toId + "')" + " AND " + COLUMN_TYPE + "=" + type + " AND "
							+ COLUMN_LOGIN_ID + "='" + WeiYuanCommon.getUserId(BMapApiApp.getInstance()) + "'" + " AND " + COLUMN_CONTENT + " like '%" + searchContent + "%' " + " AND "
							+ COLUMN_MESSAGE_TYPE + " =" + MessageType.TEXT;
				} else if (type == 300 && (toId != null && !toId.equals(""))) {
					querySql = "SELECT * FROM " + TABLE_NAME + " WHERE " + COLUMN_TO_ID + "='" + toId + "'" + " AND " + COLUMN_TYPE + "=" + type + " AND " + COLUMN_LOGIN_ID + "='"
							+ WeiYuanCommon.getUserId(BMapApiApp.getInstance()) + "'" + " AND " + COLUMN_CONTENT + " like '%" + searchContent + "%' " + " AND " + COLUMN_MESSAGE_TYPE + " ="
							+ MessageType.TEXT;
				}
			}
			...
			if (cursor != null) {
				...
				cursor.getColumnIndex(COLUMN_LAT);
				cursor.getColumnIndex(COLUMN_LNG);
				...
				do {
					...
					MainSearchEntity message = new MainSearchEntity("聊天记录", typeChat, nickname, cursor.getString(indexContent), headUrl, cursor.getLong(indexSendTime), 2, uid, searchContent, "");
					/*
					 * message.fromid = cursor.getString(indexFromId); message.fromname = cursor.getString(indexFromName); message.fromurl = cursor.getString(indexFromHead); message.toid = cursor.getString(indexToID); message.toname = cursor.getString(indexToName); message.tourl = cursor.getString(indexToHead); message.id = cursor.getString(indexMessageId); message.tag =
					 * cursor.getString(indexMessageTag); message.content = cursor.getString(indexContent); message.imgUrlS = cursor.getString(indexImgUrls); message.imgUrlL = cursor.getString(indexImgUrlL); message.voiceUrl = cursor.getString(indexVoiceUrl); message.typefile= cursor.getInt(indexMessageType); message.imgWidth = cursor.getInt(indexImgWidth); message.imgHeight =
					 * cursor.getInt(indexImgHeight); message.mLat = Double.parseDouble(cursor.getString(indexLatID)); message.mLng = Double.parseDouble(cursor.getString(indexLngID)); message.mAddress = cursor.getString(indexAddressID);
					 * 
					 * message.time = cursor.getLong(indexSendTime); message.voicetime = cursor.getInt(indexVoiceTime); message.readState = cursor.getInt(indexReadState); message.sendState = cursor.getInt(indexSendState); message.isReadVoice = cursor.getInt(indexVoiceReadState); message.systeMessage = cursor.getInt(indexSystemMessage); message.voiceString = cursor.getString(indexVoiceString);
					 * message.imageString = cursor.getString(indexImageString);
					 */
	...
	public List<MessageInfo> query(String toId, int autoID, int type) {
		...
		try {
			...
			if (type == 100) {
				if (autoID == -1) {
					querySql = "SELECT rowid, * FROM " + TABLE_NAME + " WHERE (" + COLUMN_FROM_UID + "='" + toId + "' or " + COLUMN_TO_ID + "='" + toId + "')" + " AND " + COLUMN_LOGIN_ID + "='"
							+ WeiYuanCommon.getUserId(BMapApiApp.getInstance()) + "'" + " AND " + COLUMN_TYPE + "=" + type + " ORDER BY rowid" + " DESC LIMIT 0," + WeiYuanCommon.LOAD_SIZE;
				} else {
					querySql = "SELECT rowid, * FROM " + TABLE_NAME + " WHERE (" + COLUMN_FROM_UID + "='" + toId + "' or " + COLUMN_TO_ID + "='" + toId + "')" + " AND " + COLUMN_LOGIN_ID + "='"
							+ WeiYuanCommon.getUserId(BMapApiApp.getInstance()) + "'" + " AND " + " rowid<" + autoID + " AND " + COLUMN_TYPE + "=" + type + " ORDER BY rowid" + " DESC LIMIT 0,"
							+ WeiYuanCommon.LOAD_SIZE;
				}

			} else {
				if (autoID == -1) {
					querySql = "SELECT rowid, * FROM " + TABLE_NAME + " WHERE " + COLUMN_TO_ID + "='" + toId + "' AND " + COLUMN_LOGIN_ID + "='" + WeiYuanCommon.getUserId(BMapApiApp.getInstance())
							+ "'" + " AND " + COLUMN_TYPE + "=" + type + " ORDER BY rowid" + " DESC LIMIT 0," + WeiYuanCommon.LOAD_SIZE;
				} else {
					querySql = "SELECT rowid, * FROM " + TABLE_NAME + " WHERE " + COLUMN_TO_ID + "='" + toId + "' AND " + COLUMN_LOGIN_ID + "='" + WeiYuanCommon.getUserId(BMapApiApp.getInstance())
							+ "'" + " AND " + " rowid<" + autoID + " AND " + COLUMN_TYPE + "=" + type + " ORDER BY rowid" + " DESC LIMIT 0," + WeiYuanCommon.LOAD_SIZE;
				}
	...
	public int queryUnreadCountByID(String id, int type) {
		...
		try {
			...
			String querySql = "";
			if (type == 100) {
				querySql = "SELECT rowid, * FROM " + TABLE_NAME + " WHERE " + COLUMN_FROM_UID + "='" + id + "' AND " + COLUMN_TYPE + "=100 AND " + COLUMN_READ_STATE + "=0" + " AND " + COLUMN_LOGIN_ID
						+ "='" + WeiYuanCommon.getUserId(BMapApiApp.getInstance()) + "'" + " ORDER BY " + COLUMN_SEND_TIME + " DESC ";
			} else {
				querySql = "SELECT rowid, * FROM " + TABLE_NAME + " WHERE " + COLUMN_TO_ID + "='" + id + "' AND " + COLUMN_TYPE + "=type AND " + COLUMN_READ_STATE + "=0" + " AND " + COLUMN_LOGIN_ID
						+ "='" + WeiYuanCommon.getUserId(BMapApiApp.getInstance()) + "'" + " ORDER BY " + COLUMN_SEND_TIME + " DESC ";
			}
	...
	public int queryUnreadCount() {
		...
		try {
			...
			String querySql = "";
			querySql = "SELECT rowid, * FROM " + TABLE_NAME + " WHERE " + COLUMN_READ_STATE + "=0" + " AND " + COLUMN_LOGIN_ID + "='" + WeiYuanCommon.getUserId(BMapApiApp.getInstance()) + "'";
