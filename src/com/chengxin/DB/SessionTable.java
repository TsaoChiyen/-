...
public class SessionTable {
	...
	public static final String COLUMN_MESSAGE_TYPE = "type";			//100--单聊		300--群聊   500-秘室
	...
	public boolean update(Session session, int type) {
		...
		allPromotionInfoValues.put(COLUMN_SEND_TIME, session.lastMessageTime);
		try {
			mDBStore.update(TABLE_NAME, allPromotionInfoValues, COLUMN_SESSION_ID + " = '" + session.getFromId() + "' AND " + COLUMN_MESSAGE_TYPE + "=" + type + " AND " + COLUMN_LOGIN_ID + "='" + WeiYuanCommon.getUserId(BMapApiApp.getInstance()) + "'", null);
			return true;
		} catch (SQLiteConstraintException e) {
			e.printStackTrace();
		}			
	...
	public boolean delete(String fromId, int type) {
		try {
			mDBStore.delete(TABLE_NAME, COLUMN_SESSION_ID + " = '" + fromId + "' AND " + COLUMN_MESSAGE_TYPE + "=" + type + " AND " + COLUMN_LOGIN_ID + "='" + WeiYuanCommon.getUserId(BMapApiApp.getInstance()) + "'", null);
			return true;
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public Session query(String fromId, int type){
		...
		try {
			cursor = mDBStore.rawQuery("SELECT * FROM " + TABLE_NAME + " WHERE " + COLUMN_SESSION_ID + "='" + 
					...
					+ " AND " + COLUMN_LOGIN_ID + "='" + WeiYuanCommon.getUserId(BMapApiApp.getInstance()) + "'", null);
			if (cursor != null) {
	...
	public int getTopSize(){
		...
		try {
			cursor =  mDBStore.rawQuery("SELECT * FROM " + TABLE_NAME + " WHERE " +
					COLUMN_LOGIN_ID + "='" + WeiYuanCommon.getUserId(BMapApiApp.getInstance()) + "' AND "
	...
	public int querySessionCount(int searchType) {
		...
		try {
			cursor = mDBStore.rawQuery("SELECT * FROM " + TABLE_NAME +
					" WHERE " + COLUMN_LOGIN_ID + "='" + WeiYuanCommon.getUserId(BMapApiApp.getInstance()) + "' "
