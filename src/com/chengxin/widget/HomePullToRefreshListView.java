...
public class HomePullToRefreshListView extends LinearLayout{
	...
	public HomePullToRefreshListView(Context context, AttributeSet attrs) {
		super(context, attrs);
		init(context);
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		float oldLastX = mInterceptX;
		mInterceptY = ev.getY();
		mInterceptX = ev.getX();
		float moveY = mInterceptY - oldLastY;
		float moveX = mInterceptX - oldLastX;
		...
		//	System.out.println("onInterceptTouchEvent.ACTION_MOVE");
			if (mList.getFirstVisiblePosition() == 0
					&& (mList.getChildCount() == 0 || mList.getChildAt(0)
							.getTop() == 0)) {
				if ((Math.abs(moveY) > Math.abs(moveX) && /* Math.abs(moveY) - Math.abs(moveX) > FeatureFunction.dip2px(mContext, 20)) && */(moveY > 5)) || (mState == STATE_PULL)
	...
	protected void onFinishInflate() {
		...
		mRefreshViewImage.setImageResource(R.drawable.arrow_down);
		mList = (HomeListView) findViewById(R.id.lv_weibo);
		mList.setSelected(false);
		mList.setFocusable(false);
