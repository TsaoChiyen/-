...
import com.chengxin.adapter.PagerAdapter;
...
public class ViewPager extends ViewGroup {
	...
	protected static final int SCROLL_AUTO = 0x9000;
	...
	void dataSetChanged() {
		boolean needPopulate = (this.mItems.isEmpty())
		...
		for (int i = 0; i < this.mItems.size(); i++) {
			...
			if (newPos == -2) {
				...
				newCurrItem = Math.max(0,
						Math.min(this.mCurItem, this.mAdapter.getCount() - 1));
	...
	void populate() {
		...
		lastPos = this.mItems.size() > 0 ? ((ItemInfo) this.mItems
				.get(this.mItems.size() - 1)).position : -1;
		if (lastPos < endPos) {
			lastPos++;
	...
	public void addView(View child, int index, ViewGroup.LayoutParams params) {
		if (this.mInLayout) {
			...
			child.measure(this.mChildWidthMeasureSpec,
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		this.mChildWidthMeasureSpec = View.MeasureSpec.makeMeasureSpec(
				getMeasuredWidth() - getPaddingLeft() - getPaddingRight(),
				1073741824);

		this.mChildHeightMeasureSpec = View.MeasureSpec.makeMeasureSpec(
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		case 2:
			...
			if (!this.mIsBeingDragged)
				break;
			int activePointerIndex = MotionEventCompat.findPointerIndex(ev,
					this.mActivePointerId);
			...
			// float rightBound = Math.min(this.mCurItem + 1,
			// this.mAdapter.getCount() - 1) * width;

			/*
			 * if (scrollX < leftBound) scrollX = leftBound; else if (scrollX >
			 * rightBound) { scrollX = rightBound; }
			 */
			...
			if (this.mOnPageChangeListener != null) {
				...
				this.mOnPageChangeListener.onPageScrolled(position,
		...
		case 6:
			onSecondaryPointerUp(ev);
			this.mLastMotionX = MotionEventCompat.getX(ev, MotionEventCompat
...
//		if (mIndex > mItems.size() - 1 || mIndex < 0) {
//			mIndex = 0;
//		}
//		
//		setCurrentItemInternal(mIndex, true, true);
//		
//		mIndex += 1;
//	}
//
