...
public class RegisterActivity extends BaseActivity implements OnClickListener {
	...
	private Handler mHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
				...
				if (register.mState != null) {
					if (register.mState.code == 0) {
						if (register.mLogin != null) {
							...
							WeiYuanCommon.saveLoginResult(mContext, register.mLogin);
							WeiYuanCommon.setUid(register.mLogin.uid);
						}

						Editor editor = mContext.getSharedPreferences(WeiYuanCommon.REMENBER_SHARED, 0).edit();
						editor.putBoolean(WeiYuanCommon.ISREMENBER, true);
						editor.putString(WeiYuanCommon.USERNAME, mInputNumber);
						editor.putString(WeiYuanCommon.PASSWORD, mInputPwd);
						editor.commit();
			...
			case GlobalParam.MSG_CHECK_VALID_ERROR:
				WeiYuanState validState = (WeiYuanState) msg.obj;
				if (validState == null) {
	...
	private void checkNumber() {
		...
		} else if (!WeiYuanCommon.isMobileNum(mInputNumber)) {
			isCheck = false;
		...
		new Thread() {
			public void run() {
				try {
					...
					// ++检测验证码++//
					WeiYuanState validState = WeiYuanCommon.getWeiYuanInfo().checkVerCode(mInputNumber, mInputValidCode);
					// -检测验证码-//

					if (validState != null && validState.code == 0) {// 注册
						LoginResult status = WeiYuanCommon.getWeiYuanInfo().register(mInputNumber, mInputPwd, mInputValidCode);
						WeiYuanCommon.sendMsg(mHandler, GlobalParam.REGISTER_REQUEST, status);
					} else {
						WeiYuanCommon.sendMsg(mHandler, GlobalParam.MSG_CHECK_VALID_ERROR, validState);
					}
	...
	private void getVeriCode() {
		...
		if (mInputNumber == null || mInputNumber.equals("")) {
			Toast.makeText(mContext, mContext.getResources().getString(R.string.please_input_phone_number), Toast.LENGTH_LONG).show();
			return;
		} else if (!WeiYuanCommon.isMobileNum(mInputNumber)) {
			Toast.makeText(mContext, mContext.getResources().getString(R.string.check_phone_number_hint), Toast.LENGTH_LONG).show();
			return;
		}
		new Thread() {
			public void run() {
				try {
					WeiYuanCommon.sendMsg(mBaseHandler, BASE_SHOW_PROGRESS_DIALOG, mContext.getResources().getString(R.string.get_code));
					int type = 0;

					WeiYuanState status = WeiYuanCommon.getWeiYuanInfo().getVerCode(mInputNumber, type);
